...
git clone https://github.com/alliselwah/IE7374_Group4.git
cd IE7374_Group4 # Navigate into the project root directory


Download the Dataset:

Go to the RecipeNLG Dataset on Kaggle.
...
This script will load the dataset, prepare it, and start the fine-tuning process based on the model_type, training_epochs, and training_batch_size defined in configs/model_config.yaml.

Trained models and checkpoints will be saved to the directory specified by model_checkpoints_dir in your config, inside a subdirectory corresponding to the model_type (e.g., model_checkpoints/gpt2-pytorch/).

Generating Recipes (model_runner.py)

python3 src/model_runner.py

This script will load the model specified by model_type and fine_tuned_model_path in configs/model_config.yaml.

It will then take num_inference_samples prompts from the validation set (from your full_dataset.csv) and generate new recipe instructions.

The generated outputs will be saved to outputs/generated_recipes.txt. This file will contain the input prompt, the generated instructions, and the original ground truth instructions for easy comparison.
